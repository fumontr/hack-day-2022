...
export const RoomCreateMordal = ({setMyId, setFriendId,setDialogDisplayState, setRoomCreateMordalDisplayState}) => {
  ...
  const close = async () => {
    await fetch('https://backend-dot-hack-day-2022-362804.de.r.appspot.com/rooms/' + inputRef.current.value.toString() + '/join', requestOptions)
    ...
    <div className="overflow-y-auto overflow-x-hidden fixed z-50 p-16 h-full w-full top-0 right-0 left-0 drop-shadow-2xl">
      <div className=" bg-white h-full p-20">
        ...
        <div className="flex justify-between">
          <button type="submit" onClick={close} className="my-8 flex-1 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4">さんか</button>
        </div>
      </div>
    </div>
    );
